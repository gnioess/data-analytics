(function () {
  ...
  function renderGroupedBarChart(container, opts) {
    ...
    opts.categories.forEach(function (cat, i) {
      ...
      opts.series.forEach(function (s, si) {
        ...
        if (opts.showLabels) {
          valueLabelsHtml += '<text class="value-label" x="' + (x0 + w / 2) + '" y="' + (top - 4) + '" text-anchor="middle" font-size="9">' + opts.formatValue(val) + '</text>';
        }
  ...
  function renderKPIs(kpi) {
    ...
    if (kpi.oeeMetaPrograma != null) {
      tiles.push({ label: 'OEE Meta Programa', value: fmtPct(kpi.oeeMetaPrograma, 1), icon: ICON_TARGET, accent: 'var(--series-5)' });
      tiles.push({
        label: 'Desv. OEE Programa', value: fmtSigned(kpi.desviacionOeePrograma, function (v) { return fmtPct(v, 1); }),
        icon: ICON_TREND, accent: 'var(--series-3)',
  ...
  function renderDetalleMes(anio, mesNombre) {
    ...
    if (oeeProgramaEl) {
      oeeProgramaEl.textContent = oeeProgramaMes != null ?
        'OEE Meta Programa (planta, según programa de producción de ' + mesNombre + '): ' + fmtPct(oeeProgramaMes, 1) : '';
    }
    ...
    renderGroupedBarChart(el('chartMesOEEDesglose'), {
      ...
      formatValue: function (v) { return fmtPct(v, 0); }, showLabels: true
    });
