(function () {
  ...
  function fmtPct(n, dec) {
    if (n == null) return '-';
    return (n * 100).toLocaleString('es-CL', { maximumFractionDigits: dec == null ? 2 : dec, minimumFractionDigits: dec == null ? 2 : dec }) + '%';
  }
  ...
  function parseMetas(sheets) {
    ...
    //  (c) "programa" targets (Calidad Planta Esperado / OEE Programa): a plain
    //      month-header row (no "Máquina" label, may start at any column) followed
    //      by one or two labeled rows of 12 monthly values — these are official,
    //      already-computed-by-the-plant values, not something this app derives.
    var oeeMeta = {}; // oeeMeta[label][mes] = value
    var chatarraMeta = {}; // chatarraMeta[machine] = value
    var calidadPlantaEsperado = {}; // calidadPlantaEsperado[mes] = value (oficial, calculado por programa)
    var oeePrograma = {}; // oeePrograma[mes] = value (oficial, calculado por programa de producción)
    ...
    sheets.forEach(function (aoa) {
      ...
      // (c) "programa" targets: a month-header row (Enero..Diciembre, wherever it
      // starts — this one doesn't lead with "Máquina" in col A like the OEE-meta
      // matrix above) followed by labeled rows "Calidad Planta Esperado" / "OEE
      // Programa", each one value per month in the same columns as the header.
      for (var r3 = 0; r3 < aoa.length; r3++) {
        var row3 = aoa[r3] || [];
        var monthCols3 = mesKeys.map(function (mk) {
          for (var c = 0; c < row3.length; c++) { if (normKey(row3[c]) === mk) return c; }
          return null;
        });
        if (monthCols3.filter(function (c) { return c != null; }).length < 6) continue;
        for (var rr3 = r3 + 1; rr3 <= r3 + 8 && rr3 < aoa.length; rr3++) {
          var label3 = normKey((aoa[rr3] || [])[0]);
          if (label3 === normKey('Calidad Planta Esperado')) {
            MESES.forEach(function (m, mi) { if (monthCols3[mi] != null) calidadPlantaEsperado[m] = toNum((aoa[rr3] || [])[monthCols3[mi]]); });
          } else if (label3 === normKey('OEE Programa')) {
            MESES.forEach(function (m, mi) { if (monthCols3[mi] != null) oeePrograma[m] = toNum((aoa[rr3] || [])[monthCols3[mi]]); });
          }
        }
      }
    });
    return { oeeMeta: oeeMeta, chatarraMeta: chatarraMeta, calidadPlantaEsperado: calidadPlantaEsperado, oeePrograma: oeePrograma };
  }
  ...
  function makeEngine(D) {
    ...
    function pptoProdVal(maquina, mesNombre) {
      // un 0 acá casi siempre es una celda de presupuesto sin llenar todavía (no un
      // presupuesto real de cero kilos) — tratarlo como dato real aplana la línea de
      // meta en 0 para los meses futuros en vez de simplemente no dibujarla.
      var m = D.pptoProduccion[maquina];
      if (!m) return null;
      return zeroToNull(m[mesNombre]);
    }
    ...
    // Valores oficiales "por programa" (calculados por el plan de producción del
    // mes, no una aproximación de este motor) — solo existen a nivel planta, sin
    // desglose por máquina, y no todos los Excel los traen todavía.
    function calidadPlantaEsperadoVal(mesNombre) {
      var v = D.metas.calidadPlantaEsperado[mesNombre];
      return isNum(v) ? v : null;
    }
    function oeeProgramaVal(mesNombre) {
      var v = D.metas.oeePrograma[mesNombre];
      return isNum(v) ? v : null;
    }
    ...
    function kpiHeader(anio, resumen) {
      ...
      // meta calculada total: por mes, prefiere el "Calidad Planta Esperado" oficial
      // (hoja Metas); si ese mes no lo trae, cae al componente de máquinas (arriba) +
      // el estándar de Braner/desorillado. Meses sin ninguna de las dos señales se
      // excluyen del promedio (no se usa la meta fija administrativa acá — para eso
      // ya está el tile "Meta Administrativa" — así el tile queda en "-" cuando este
      // Excel no trae ni tabla de rechazo ni el dato oficial, igual que antes).
      var numCalcTotal = 0, denCalcTotal = 0;
      resumen.forEach(function (m) {
        var oficial = calidadPlantaEsperadoVal(m.mes);
        var dinamica = chatarraPorEspesorTarget(anio, m.mes);
        var val = oficial != null ? oficial : (dinamica != null ? dinamica + META_CHATARRA_BRANER : null);
        if (val == null) return;
        var w = (m.totalProduccion || 0) + (m.totalChatarraReal || 0);
        numCalcTotal += val * w; denCalcTotal += w;
      });
      var metaChatarraCalculadaTotal = denCalcTotal ? numCalcTotal / denCalcTotal : null;
      ...
      // OEE meta programa: promedio ponderado (mismo criterio de arriba) del "OEE
      // Programa" oficial (hoja Metas, calculado por el usuario en función del
      // programa de producción) para los meses en que el Excel lo trae.
      var numOeeProg = 0, denOeeProg = 0;
      resumen.forEach(function (m) {
        var prog = oeeProgramaVal(m.mes);
        if (isNum(prog)) {
          var w = (m.totalProduccion || 0) + (m.totalChatarraReal || 0);
          numOeeProg += prog * w; denOeeProg += w;
        }
      });
      var oeeMetaPrograma = denOeeProg ? numOeeProg / denOeeProg : null;
      var desviacionOeePrograma = (oeeMetaPrograma != null && oeePlantaGlobal != null) ?
        oeePlantaGlobal - oeeMetaPrograma : null;
      ...
      return {
        ...
        oeeMetaPrograma: oeeMetaPrograma, desviacionOeePrograma: desviacionOeePrograma
      };
    }
    ...
    // Meta total de Calidad Planta para un mes: prefiere el valor oficial
    // "Calidad Planta Esperado" (calculado por el programa de producción, hoja
    // Metas) cuando el Excel lo trae — es la fuente de verdad real, no una
    // aproximación. Si ese mes no está (todavía sin cerrar, o el Excel no trae
    // la tabla), cae a la meta por espesor de este motor + el estándar de Braner.
    function metaCalidadPlantaTotal(anio, mesNombre) {
      var oficial = calidadPlantaEsperadoVal(mesNombre);
      if (oficial != null) return { valor: oficial, esOficial: true };
      var metaDinamica = chatarraPorEspesorTarget(anio, mesNombre);
      var metaMaq = metaDinamica != null ? metaDinamica : META_CHATARRA_MAQUINAS;
      return { valor: metaMaq + META_CHATARRA_BRANER, esOficial: false, esDinamica: metaDinamica != null };
    }
    ...
    function presupuesto(anio, resumen) {
      ...
      var chatarra = resumen.map(function (m) {
        ...
        var metaTotalInfo = metaCalidadPlantaTotal(anio, m.mes);
        var metaCalculada = pct == null ? null : metaTotalInfo.valor;
        ...
        return {
          ...
          metaEsDinamica: !!metaTotalInfo.esDinamica, metaEsOficial: !!metaTotalInfo.esOficial
        };
      });
    ...
    function calidadPlantaDetalle(anio) {
      ...
      return resumen.map(function (m) {
        ...
        // el total prefiere el "Calidad Planta Esperado" oficial (hoja Metas) —
        // el desglose máquinas/Braner de abajo sigue siendo el cálculo propio de
        // este motor, ya que el valor oficial no viene separado en componentes
        var metaTotalInfo = metaCalidadPlantaTotal(anio, m.mes);
        var metaTotal = metaTotalInfo.valor;
        ...
        return {
          ...
          desvTotal: desvTotal, metaEsDinamica: metaDinamica != null, metaEsOficial: metaTotalInfo.esOficial
        };
      });
    }
    ...
    function skuBuscador(anio, opts) {
      ...
      // meta calculada por fila: la meta por espesor de esa máquina, para el mes
      // filtrado (o el promedio de los meses con dato, si se ve el año completo).
      // Cachear por (mes, máquina) — son a lo más ~100 combinaciones aunque haya
      // cientos de filas, así que evita recalcular la tabla de rechazo por fila.
      var metaCache = {};
      function metaFor(mesNombre, maquina) {
        var key = mesNombre + '|' + maquina;
        if (!(key in metaCache)) metaCache[key] = chatarraPorEspesorTarget(anio, mesNombre, maquina);
        return metaCache[key];
      }
      rows.forEach(function (r) {
        if (opts.mes) {
          r.metaCalculada = metaFor(opts.mes, r.maquina);
        } else {
          var vals = MESES.map(function (m) { return metaFor(m, r.maquina); }).filter(isNum);
          r.metaCalculada = vals.length ? sum(vals) / vals.length : null;
        }
        r.desviacionCalculada = (r.metaCalculada != null && r.chatPct != null) ? r.metaCalculada - r.chatPct : null;
      });
      return rows;
    }
  ...
  function renderBulletChart(container, opts) {
    ...
    // meta como línea punteada delgada con su valor en cada punto, en vez de un
    // trazo suelto por categoría — mismo lenguaje visual que el resto de la app
    var targetHtml = '', targetLabelsHtml = '';
    if (opts.targets.some(isNum)) {
      var tPath = '', started = false;
      opts.targets.forEach(function (tgt, i) {
        if (!isNum(tgt)) { started = false; return; }
        tPath += (started ? 'L' : 'M') + cxOf(i) + ',' + y(tgt);
        started = true;
      });
      targetHtml = '<path d="' + tPath + '" fill="none" stroke="var(--text-primary)" stroke-width="1.8" stroke-dasharray="5 3" opacity=".8"></path>';
      opts.targets.forEach(function (tgt, i) {
        if (!isNum(tgt)) return;
        var cx = cxOf(i), yt = y(tgt);
        targetHtml += '<circle cx="' + cx + '" cy="' + yt + '" r="2.6" fill="var(--text-primary)"></circle>';
        targetLabelsHtml += '<text class="value-label" x="' + cx + '" y="' + (yt - 8) + '" text-anchor="middle" font-size="9.5" fill="var(--text-primary)">' + (opts.formatTarget ? opts.formatTarget(tgt) : opts.formatValue(tgt)) + '</text>';
      });
    }
    ...
    var svg = '<div class="chart-wrap">' + legendHtml +
      ...
      gridHtml + labelsHtml + barsHtml + targetHtml + valueLabelsHtml + targetLabelsHtml + xLabelsHtml +
      '</svg>' +
  ...
  function renderStackedBarChart(container, opts) {
      ...
      opts.series.forEach(function (s, si) {
        ...
        if (hDraw >= 26) {
          stackLabelsHtml += '<text class="mix-stack-label" x="' + cx + '" y="' + (top + hDraw / 2 - 3) + '" text-anchor="middle">' + escapeHtml(s.name.replace(' mm', 'mm')) + '</text>' +
            '<text class="mix-stack-label" x="' + cx + '" y="' + (top + hDraw / 2 + 9) + '" text-anchor="middle">' + fmtPct(v / total, 2) + '</text>';
        } else if (hDraw >= 13) {
    ...
    container.querySelectorAll('.bar').forEach(function (barEl) {
      ...
      barEl.addEventListener('mousemove', function () {
        ...
        tip.style.opacity = 1;
        tip.innerHTML = '<strong>' + seg.cat + '</strong><br>' + seg.name + ': ' + fmtPct(seg.v, 2);
        barEl.classList.add('bar-active');
      });
  ...
  function renderGroupedBarChart(container, opts) {
    ...
    opts.categories.forEach(function (cat, i) {
      ...
      opts.series.forEach(function (s, si) {
        ...
        if (opts.showLabels) {
          // las barras de un grupo quedan angostas (varias series por categoría),
          // así que la etiqueta sobre la barra usa un formateador más compacto que
          // el de los ejes/tooltip si se provee uno — la precisión completa igual
          // queda disponible en la tabla de datos que acompaña a este gráfico.
          var labelFmt = opts.formatLabel || opts.formatValue;
          valueLabelsHtml += '<text class="value-label" x="' + (x0 + w / 2) + '" y="' + (top - 4) + '" text-anchor="middle" font-size="9">' + labelFmt(val) + '</text>';
        }
  ...
  function renderDonutChart(container, opts) {
    ...
    items.forEach(function (it, i) {
      ...
      if (frac >= 0.04) {
        ...
        labels += '<text class="stack-label" x="' + lx + '" y="' + (ly + 3.5) + '" text-anchor="middle">' + fmtPct(frac, 2) + '</text>';
      }
    ...
    var legend = '<div style="display:flex;flex-direction:column;gap:7px;justify-content:center;">' + pts.map(function (p, i) {
      ...
      return '<span class="sw" style="display:inline-flex;align-items:center;gap:7px;font-size:12px;color:var(--text-secondary);">' +
        ...
        '<span>' + escapeHtml(p.label) + ' <span class="subtle">' + fmtPct(p.frac, 2) + '</span></span></span>';
    }).join('') + '</div>';
    ...
    container.querySelectorAll('path.bar').forEach(function (elm) {
      ...
      elm.addEventListener('mousemove', function (ev) {
        ...
        tip.style.opacity = 1;
        tip.innerHTML = '<strong>' + escapeHtml(p.label) + '</strong><br>' + opts.formatValue(p.value) + ' — ' + fmtPct(p.frac, 2);
      });
    ...
    container.querySelectorAll('.bar').forEach(function (elm) {
      ...
      elm.addEventListener('mousemove', function () {
        ...
        tip.innerHTML = '<strong>#' + (i + 1) + ' ' + escapeHtml(p.it.label) + '</strong><br>' +
          fmtInt(p.it.kg) + ' kg — ' + fmtPct(p.it.share, 2) + ' del total<br>acumulado: ' + fmtPct(p.it.cum, 2);
      });
      elm.addEventListener('mouseleave', function () { tip.style.opacity = 0; });
    });
  }
  ...
  function renderBubbleChart(container, opts) {
    ...
    if (opts.metaLine) opts.metaLine.values.forEach(function (v) { if (isNum(v)) allVals.push(v); });
    var maxV = allVals.length ? Math.max.apply(null, allVals) : 1;
    maxV = maxV * 1.3 || 1;
    ...
    // meta calculada: línea base sólida delgada, verde clara, que corre borde a
    // borde de cada banda de mes (no solo punto a punto por el centro) — así se
    // ve como una referencia continua a lo largo de todo el gráfico en vez de un
    // segmento corto perdido en el medio de la banda.
    var META_LINE_COLOR = '#8bc34a';
    var metaLineHtml = '', metaLegendHtml = '';
    if (opts.metaLine) {
      var mSegs = '';
      opts.metaLine.values.forEach(function (v, i) {
        if (!isNum(v)) return;
        var left = padL + bandW * i, right = padL + bandW * (i + 1);
        var my = y(v);
        mSegs += 'M' + left + ',' + my + 'L' + right + ',' + my + ' ';
      });
      metaLineHtml = '<path d="' + mSegs + '" fill="none" stroke="' + META_LINE_COLOR + '" stroke-width="1.5" opacity=".9"></path>';
      opts.metaLine.values.forEach(function (v, i) {
        if (!isNum(v)) return;
        var mx = xBase(i), my = y(v);
        metaLineHtml += '<text class="value-label" x="' + mx + '" y="' + (my - 8) + '" text-anchor="middle" font-size="9.5" fill="' + META_LINE_COLOR + '">' + opts.formatValue(v) + '</text>';
      });
      metaLegendHtml = '<span class="sw"><span class="dot" style="background:' + META_LINE_COLOR + '"></span>' + escapeHtml(opts.metaLine.name) + '</span>';
    }
    ...
    var bubbleLegendHtml = metaLegendHtml ? '<div class="chart-legend">' + metaLegendHtml + '</div>' : '';
    container.innerHTML = '<div class="chart-wrap">' + bubbleLegendHtml +
      '<svg class="chart" viewBox="0 0 ' + W + ' ' + H + '" id="svg' + chartUid + '">' +
      '<line class="baseline" x1="' + padL + '" x2="' + (W - padR) + '" y1="' + baseline + '" y2="' + baseline + '"></line>' +
      gridHtml + monthSepHtml + labelsHtml + leadersHtml + bubblesHtml + centerLabelsHtml + nameLabelsHtml + metaLineHtml + xLabelsHtml +
      '</svg><div class="chart-tooltip" id="' + tipId + '"></div></div>';
  ...
  function fmtPctTick(v) { return (v * 100).toLocaleString('es-CL', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + '%'; }
  ...
  function renderKPIs(kpi) {
    var tiles = [
      { label: 'OEE Planta Global', value: fmtPct(kpi.oeePlantaGlobal, 2), icon: ICON_GAUGE, accent: 'var(--series-5)' },
      ...
      { label: 'Meta Administrativa', value: fmtPct(kpi.metaChatarra, 2), icon: ICON_TARGET, accent: 'var(--series-4)' },
      {
    ...
    if (kpi.oeeMetaPrograma != null) {
      tiles.push({ label: 'OEE Meta Programa', value: fmtPct(kpi.oeeMetaPrograma, 2), icon: ICON_TARGET, accent: 'var(--series-5)' });
      tiles.push({
        label: 'Desv. OEE Programa', value: fmtSigned(kpi.desviacionOeePrograma, function (v) { return fmtPct(v, 2); }),
        icon: ICON_TREND, accent: 'var(--series-3)',
        deltaClass: deltaClass(kpi.desviacionOeePrograma, true),
  ...
  function renderResumen(anio) {
    ...
    var rows = [
      ...
      { label: 'OEE Planta Mensual', unit: '%', values: resumen.map(function (m) { return m.oeeMensual; }), fmt: function (v) { return fmtPct(v, 2); } }
    ];
  ...
  function renderPresupuesto(anio, resumen) {
    ...
    // orden de preferencia de la meta calculada: oficial "Calidad Planta Esperado"
    // (programa de producción) > dinámica por espesor (este motor) > 4,6% fijo —
    // se avisa en el subtítulo cuál está activo, ya que no todos los Excel traen
    // las dos primeras.
    var hayMetaOficialPpto = ppto.chatarra.some(function (m) { return m.metaEsOficial; });
    var hayMetaDinamica = ppto.chatarra.some(function (m) { return m.metaEsDinamica; });
    el('chatPptoCap').textContent = hayMetaOficialPpto ?
      '% chatarra planta vs. meta oficial "Calidad Planta Esperado" (programa de producción; meses sin ese dato usan la meta calculada por espesor o 4,6% fijo)' :
      (hayMetaDinamica ?
    ...
    renderComboChart(el('chartChatPpto'), {
      ...
      metaName: hayMetaOficialPpto ? 'Meta Programa' : (hayMetaDinamica ? 'Meta por espesor' : 'Meta (4,6%)'), metaValues: ppto.chatarra.map(function (m) { return m.meta; }),
    ...
    var rows = [
      ...
      { label: 'Desviación Ppto vs Real', unit: '%', values: ppto.produccion.map(function (m) { return m.desvPct; }), fmt: function (v) { return fmtPct(v, 2); } },
      ...
      { label: 'Meta Administrativa', unit: '%', values: ppto.chatarra.map(function (m) { return m.metaAdministrativa; }), fmt: function (v) { return fmtPct(v, 2); } },
      ...
      { label: 'Meta Calculada', unit: '%', values: ppto.chatarra.map(function (m) { return m.metaCalculada; }), fmt: function (v) { return fmtPct(v, 2); } },
  ...
  function renderOeeCalidad(anio) {
    ...
    var categories, oeeReal, oeeMeta, calReal, calEsperado, calDesvMaq, calDesvBran, calDesvTotal, hayDesglose, hayMetaDinamica, hayOeePrograma, hayMetaOficial;

    if (!maquina) {
      var resumen = STATE.engine.resumenPlanta(anio);
      categories = resumen.map(function (m) { return m.mesAbbr; });
      oeeReal = resumen.map(function (m) { return m.oeeMensual; });
      // "OEE Programa" (calculado por el plan de producción del mes, oficial) es
      // preferible a la meta administrativa fija cuando el Excel lo trae
      oeeMeta = resumen.map(function (m) {
        var prog = STATE.engine.oeeProgramaVal(m.mes);
        return prog != null ? prog : STATE.engine.oeeMetaVal('OEE Planta', m.mes);
      });
      hayOeePrograma = resumen.some(function (m) { return STATE.engine.oeeProgramaVal(m.mes) != null; });
      ...
      hayMetaOficial = cal.some(function (m) { return m.metaEsOficial; });
      hayDesglose = true;
    } else {
    ...
    el('oeeCalidadOeeCap').textContent = 'OEE real vs. ' + (hayOeePrograma ? 'meta por programa de producción' : 'meta administrativa') + ' — ' + maquinaLabel;
    renderTopLineBottomBarChart(el('chartOeeCalidadOEE'), {
      categories: categories,
      lines: [
        { name: 'OEE Real', color: 'var(--series-2)', values: oeeReal },
        { name: hayOeePrograma ? 'OEE Meta Programa' : 'OEE Meta', color: 'var(--series-5)', values: oeeMeta }
      ],
    ...
    if (hayDesglose) {
      el('oeeCalidadCalCap').textContent = hayMetaOficial ?
        'Calidad Planta: chatarra máquinas + desorillado Braner vs. meta oficial "Calidad Planta Esperado" (programa de producción)' :
        (hayMetaDinamica ?
    ...
    if (mesFiltro) {
      ...
      mesCapEl.innerHTML = '<strong>' + mesFiltro + '</strong> — OEE real ' + fmtPct(oeeReal[idx], 2) + ' vs. meta ' + fmtPct(oeeMeta[idx], 2) +
        ' (desv. ' + fmtSigned(oeeDesv[idx], function (v) { return fmtPct(v, 2); }) + ')' +
  ...
  function renderDetalleMes(anio, mesNombre) {
    ...
    // "OEE Programa" es un valor único de planta (hoja Metas), no viene desglosado
    // por máquina, así que no encaja como línea en un gráfico por máquina — se
    // muestra como referencia de texto junto al gráfico.
    var oeeProgramaMes = STATE.engine.oeeProgramaVal(mesNombre);
    var oeeProgramaEl = el('oeeProgramaNote');
    if (oeeProgramaEl) {
      oeeProgramaEl.textContent = oeeProgramaMes != null ?
        'OEE Meta Programa (planta, según programa de producción de ' + mesNombre + '): ' + fmtPct(oeeProgramaMes, 2) : '';
    }
    ...
    renderGroupedBarChart(el('chartMesOEEDesglose'), {
      ...
      formatValue: function (v) { return fmtPct(v, 2); },
      formatLabel: function (v) { return fmtPct(v, 0); }, showLabels: true
    });
    ...
    renderBulletChart(el('chartMesAporteOEE'), {
      ...
      formatValue: function (v) { return fmtPct(v, 2); }, barColorVar: 'var(--series-5)',
    ...
    renderMachineTable(el('detalleMesOEE'), 'Indicadores OEE — ' + mesNombre, D.machines ? D.machines : maquinas.slice(0, -1), [
      { label: 'Disponibilidad', unit: '%', values: det.columnas.filter(function(c){return !c.isBraner;}).map(function (c) { return c.disponibilidad; }), fmt: function (v) { return fmtPct(v, 2); } },
      { label: 'Calidad', unit: '%', values: det.columnas.filter(function(c){return !c.isBraner;}).map(function (c) { return c.calidad; }), fmt: function (v) { return fmtPct(v, 2); } },
      { label: 'Ritmo', unit: '%', values: det.columnas.filter(function(c){return !c.isBraner;}).map(function (c) { return c.ritmo; }), fmt: function (v) { return fmtPct(v, 2); } },
      { label: 'OEE Máquina', unit: '%', values: det.columnas.filter(function(c){return !c.isBraner;}).map(function (c) { return c.oee; }), fmt: function (v) { return fmtPct(v, 2); }, total: true },
      { label: 'OEE Meta', unit: '%', values: det.columnas.filter(function(c){return !c.isBraner;}).map(function (c) { return c.oeeMeta; }), fmt: function (v) { return fmtPct(v, 2); } },
      { label: 'Desviación OEE', unit: '%', values: det.columnas.filter(function(c){return !c.isBraner;}).map(function (c) { return c.desviacionOEE; }), fmt: function (v) { return fmtPct(v, 2); } },
  ...
  function renderDetalleAnio(anio, maquina) {
    ...
    var rows = [
      { label: 'Disponibilidad', unit: '%', values: det.meses.map(function (m) { return m.disponibilidad; }), fmt: function (v) { return fmtPct(v, 2); } },
      { label: 'Calidad', unit: '%', values: det.meses.map(function (m) { return m.calidad; }), fmt: function (v) { return fmtPct(v, 2); } },
      { label: 'Ritmo', unit: '%', values: det.meses.map(function (m) { return m.ritmo; }), fmt: function (v) { return fmtPct(v, 2); } },
      { label: 'OEE Máquina', unit: '%', values: det.meses.map(function (m) { return m.oee; }), fmt: function (v) { return fmtPct(v, 2); }, total: true },
      { label: 'OEE Meta', unit: '%', values: det.meses.map(function (m) { return m.oeeMeta; }), fmt: function (v) { return fmtPct(v, 2); } },
      { label: 'Desviación OEE', unit: '%', values: det.meses.map(function (m) { return m.desviacionOEE; }), fmt: function (v) { return fmtPct(v, 2); } },
      { label: 'Horas de Atraso / Adelanto', unit: 'h', values: det.meses.map(function (m) { return m.horasAtraso; }) }
    ];
    renderMonthlyTable(el('detalleAnioOEE'), 'OEE — ' + maquina + (det.oeePonderado != null ? ' (ponderado ' + fmtPct(det.oeePonderado, 2) + ')' : ''), rows);
  }
  ...
  function renderEspesor(anio) {
    ...
    var pctRows = esp.pctTable.map(function (r) { return { label: r.espesor + ' mm', values: r.meses.map(zeroToNull), fmt: function (v) { return fmtPct(v, 2); } }; });
    pctRows.push({ label: 'Total', values: esp.totalPorMes.map(function (v) { return v ? 1 : null; }), fmt: function (v) { return fmtPct(v, 2); }, total: true });
  ...
  function renderTorreControl(anio, mesNombre) {
    ...
    var pct1 = function (v) { return fmtPct(v, 2); };
    ...
    // fila de planta: OEE Programa es un valor único de planta (no viene desglosado
    // por máquina en el Excel), así que se muestra aparte, al pie de la tabla
    var oeeProg = STATE.engine.oeeProgramaVal(mesNombre);
    if (oeeProg != null) {
      var resMes = STATE.engine.resumenPlanta(anio).filter(function (m) { return m.mes === mesNombre; })[0];
      var oeeRealPlanta = resMes ? resMes.oeeMensual : null;
      var estadoOeeProg = (oeeRealPlanta == null) ? 'sindato' : (oeeRealPlanta >= oeeProg ? 'ok' : 'alerta');
      html += '<tr class="total">' +
        '<td>Planta <span class="subtle">OEE Programa</span></td>' +
        '<td class="dash">-</td><td class="dash">-</td><td class="dash">-</td><td class="dash">-</td><td class="dash">-</td><td class="dash">-</td>' +
        tdv(oeeRealPlanta, pct1, 'group-sep') + tdv(oeeProg, pct1) +
        desvTd(oeeRealPlanta, oeeProg, false, pct1) +
        '<td>' + pill(estadoOeeProg) + '</td>' +
        '<td class="group-sep dash">-</td><td class="dash">-</td><td class="dash">-</td><td class="dash">-</td>' +
        '</tr>';
    }
    html += '</tbody></table>';
  ...
  var CRITICOS_FMT = {
    kg: fmtInt,
    pct1: function (v) { return fmtPct(v, 2); },
      ...
      tipos.forEach(function (tipo) {
        ...
        items.forEach(function (it) {
          ...
          html += '<tr>' +
            ...
            '<td class="' + (it.gravedad > 0 ? 'neg' : 'pos') + '">' + fmtSigned(it.desvRel, function (v) { return fmtPct(v, 2); }) + '</td>' +
  ...
  function renderProyeccion(anio) {
    ...
    p.maquinas.forEach(function (m) {
      html += '<tr>' +
        ...
        tdv(m.desvPct, function (v) { return fmtSigned(v, function (x) { return fmtPct(x, 2); }); }) +
    ...
    html += '<tr class="total"><td>Total Planta</td>' +
      ...
      tdv(p.total.desvPct, function (v) { return fmtSigned(v, function (x) { return fmtPct(x, 2); }); }) +
  ...
  function proyDeltaSub(desvPct, favorableLabel, contraLabel) {
    ...
    var good = desvPct >= 0;
    return { cls: good ? 'good' : 'bad', text: fmtSigned(desvPct, function (v) { return fmtPct(v, 2); }) + ' ' + (good ? favorableLabel : contraLabel) };
  }
  ...
  function renderProyecciones(anio) {
    ...
    el('proyMesChips').innerHTML = [
      ...
      proyChip('OEE', oeeCierreVal != null ? fmtPct(oeeCierreVal, 2) : '-', proyDeltaSub(desvMesOee, 'sobre meta', 'bajo meta'), 'var(--series-2)')
    ].join('');
    ...
    el('proyAnioChips').innerHTML = [
      ...
      proyChip('OEE proyectado', a.oee != null ? fmtPct(a.oee, 2) : '-', proyDeltaSub(a.desvOeePct, 'sobre meta', 'bajo meta'), 'var(--series-2)')
    ].join('');
  ...
  function renderInsumos() {
    ...
    renderRankedBarChart(el('chartInsumosSobrecosto'), {
      ...
      formatValue: function (v) { return fmtPct(v, 2); }, colorVar: 'var(--series-6)'
    });
  ...
  function renderInsumoMovimiento(categoria, tableId, capId, maquina, mes, filtroTxt) {
    ...
    function deltaCell(pct, esPrimeraCompra, sinDatos, judged) {
      ...
      return '<td class="delta ' + cls + '">' + arrow + ' ' + fmtPct(Math.abs(pct), 2) + '</td>';
    }
  ...
  function renderCostosCategoria(categoria, chartId, capId, maquina, maquinaTxt) {
    ...
    if (mesActualRow && mesActualRow.desviacionPct != null) {
      parts.push('desv. vs ppto: ' + fmtSigned(mesActualRow.desviacionPct, function (v) { return fmtPct(v, 2); }));
    }
  ...
  function renderSkuBuscador() {
    ...
    var metaMeses = mesIdx >= 0 ? [STATE.sku.mes] : MESES;
    var metaLineValues = metaMeses.map(function (mesNombre) {
      return STATE.engine.chatarraPorEspesorTarget(STATE.year, mesNombre, STATE.sku.maquina || undefined);
    });
    var bubbleInfo = renderBubbleChart(el('chartSkuBubble'), {
      ...
      metaLine: metaLineValues.some(isNum) ? { name: 'Meta calculada', values: metaLineValues } : null,
      formatValue: function (v) { return fmtPct(v, 2); },
    ...
    var pct2 = function (v) { return fmtPct(v, 2); };
    // positivo = chatarra real por debajo de la meta calculada (favorable)
    function skuDesvCell(v) {
      if (v == null) return '<td class="dash">-</td>';
      return '<td class="' + (v < 0 ? 'neg' : 'pos') + '">' + fmtSigned(v, pct2) + '</td>';
    }
    var html = '<table class="wide"><thead><tr><th>Descripción</th><th>Máquina</th><th>Mes</th><th>Producción (kg)</th><th>Chatarra (kg)</th><th>Chatarra %</th><th>Meta Calculada</th><th>Desviación</th></tr></thead><tbody>';
    shown.forEach(function (r) {
      html += '<tr>' +
        ...
        tdv(r.chatPct, pct2) + tdv(r.metaCalculada, pct2) +
        skuDesvCell(r.desviacionCalculada) +
        '</tr>';
    });
    html += '</tbody></table>';
  ...
  function driveDownloadFile(fileId, name, mimeType, ctx) {
    ...
    // "cache: no-store" + un parámetro que cambia en cada click asegura que esto
    // nunca sirva una respuesta cacheada por el navegador para esta misma URL —
    // "Actualizar desde Drive" tiene que traer los bytes reales de ahora, siempre.
    ...
    fetch(url, { headers: { Authorization: 'Bearer ' + driveAccessToken }, cache: 'no-store' })
      .then(function (res) {
      ...
      .then(function (buf) {
        ...
        driveSaveFile({ id: fileId, name: name, mimeType: mimeType, ts: Date.now() });
        driveUpdateRefreshUi();
        // en el sidebar driveUpdateRefreshUi ya deja un mensaje persistente con el
        // archivo conectado — no lo pises con un hint vacío. En la pantalla de
        // conexión inicial (dz) sí se limpia, ahí es solo un mensaje de estado.
        if (ctx !== 'side') driveHint('', false, ctx);
  ...
  function driveUpdateRefreshUi() {
    ...
    if (saved) {
      ...
      // igual que dos archivos distintos en Drive pueden compartir el mismo
      // nombre — mostrar cuándo se leyó por última vez ayuda a notar si "Actualizar
      // desde Drive" sigue trayendo una copia vieja en vez de la que se acaba de editar
      var hint = el('driveRefreshHint');
      if (hint) {
        var when = saved.ts ? new Date(saved.ts).toLocaleString('es-CL', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' }) : '';
        hint.textContent = 'Conectado a "' + saved.name + '"' + (when ? ' — última lectura ' + when : '');
