(function () {
  ...
  function renderBubbleChart(container, opts) {
    ...
    // meta calculada: línea punteada delgada sobre el mismo eje sqrt, con su valor
    // en cada punto — igual criterio visual que las demás líneas de meta de la app
    var metaLineHtml = '', metaLegendHtml = '';
    if (opts.metaLine) {
      var mPath = '', mStarted = false;
      opts.metaLine.values.forEach(function (v, i) {
        if (!isNum(v)) { mStarted = false; return; }
        mPath += (mStarted ? 'L' : 'M') + xBase(i) + ',' + y(v);
        mStarted = true;
      });
      metaLineHtml = '<path d="' + mPath + '" fill="none" stroke="var(--text-primary)" stroke-width="1.8" stroke-dasharray="5 3" opacity=".8"></path>';
      opts.metaLine.values.forEach(function (v, i) {
        if (!isNum(v)) return;
        var mx = xBase(i), my = y(v);
        metaLineHtml += '<circle cx="' + mx + '" cy="' + my + '" r="2.6" fill="var(--text-primary)"></circle>' +
          '<text class="value-label" x="' + mx + '" y="' + (my - 8) + '" text-anchor="middle" font-size="9.5" fill="var(--text-primary)">' + opts.formatValue(v) + '</text>';
      });
      metaLegendHtml = '<span class="sw"><span class="dot" style="background:var(--text-primary)"></span>' + escapeHtml(opts.metaLine.name) + '</span>';
    }
